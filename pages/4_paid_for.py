...
def display_progress():
    ...
    # Paid For lantern (highlighted/active)
...
st.markdown('<h2 style="color: #3498db;">Question 4</h2>', unsafe_allow_html=True)
st.markdown('<a href="#" style="color: #3498db; text-decoration: underline;">Ikigai Chart</a>', unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)

# Main question
st.markdown('<h1 style="margin-bottom: 20px;">What can you be PAID FOR?</h1>', unsafe_allow_html=True)

# User input section
st.markdown('<p style="margin-bottom: 10px;">List careers, services, or products that people would pay you for:</p>', unsafe_allow_html=True)

# Text area for user input
user_input = st.text_area("Paid opportunities", key="paid_for_input", height=150)
...
if st.button("Get Suggestions", key="get_suggestions"):
    if user_input:
        with st.spinner("Generating suggestions..."):
            suggestions = get_ai_suggestions(user_input, "paid_for")
            if suggestions:
...
with col1:
    st.button("Back", key="back_button", on_click=lambda: switch_page("world needs"))
with col2:
    if st.button("Generate Ikigai Chart", key="next_button"):
        # Save responses
        if user_input:
            st.session_state.responses['paid_for'] = [item.strip() for item in user_input.split('\n') if item.strip()]
            st.session_state.progress['paid_for_complete'] = True
            # Create a new page for the Ikigai chart
            switch_page("ikigai_chart")
        else:
            st.warning("Please enter at least one thing you can be paid for before proceeding.")
