...
def display_progress():
    ...
    # World Needs lantern (highlighted/active)
...
st.markdown('<h2 style="color: #3498db;">Question 3</h2>', unsafe_allow_html=True)
st.markdown('<a href="#" style="color: #3498db; text-decoration: underline;">Ikigai Chart</a>', unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)

# Main question
st.markdown('<h1 style="margin-bottom: 20px;">What does the WORLD NEED?</h1>', unsafe_allow_html=True)

# User input section
st.markdown('<p style="margin-bottom: 10px;">List problems, challenges, or opportunities where you could make a difference:</p>', unsafe_allow_html=True)

# Text area for user input
user_input = st.text_area("World needs", key="world_needs_input", height=150)
...
if st.button("Get Suggestions", key="get_suggestions"):
    if user_input:
        with st.spinner("Generating suggestions..."):
            suggestions = get_ai_suggestions(user_input, "world_needs")
            if suggestions:
...
with col1:
    st.button("Back", key="back_button", on_click=lambda: switch_page("good at"))
with col2:
    if st.button("Next", key="next_button"):
        # Save responses
        if user_input:
            st.session_state.responses['world_needs'] = [item.strip() for item in user_input.split('\n') if item.strip()]
            st.session_state.progress['world_needs_complete'] = True
            switch_page("paid for")
        else:
            st.warning("Please enter at least one thing the world needs before proceeding.")
