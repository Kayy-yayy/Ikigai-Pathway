...
# Page configuration
st.set_page_config(
    page_title="What the World Needs | Ikigai Pathway",
    page_icon="🏮",
    layout="wide",
...
if 'world_needs_inputs' not in st.session_state:
    st.session_state.world_needs_inputs = [""] * 5  # 5 input fields
    
if 'world_needs_suggestions' not in st.session_state:
    st.session_state.world_needs_suggestions = [[] for _ in range(5)]  # Suggestions for each input
...
def get_suggestions(input_text, index):
    if input_text and len(input_text) > 3:
        suggestions = get_ai_suggestions(input_text, "world_needs")
        st.session_state.world_needs_suggestions[index] = suggestions
        return suggestions
    return []

# Function to add suggestion to input
def add_suggestion_to_input(suggestion, index):
    st.session_state.world_needs_inputs[index] = suggestion

# Main content
st.markdown('<h1 class="main-header">What the <span class="indigo-blue">World Needs</span></h1>', unsafe_allow_html=True)
...
st.markdown(
    ...
    <div class="centered-content">
        <p>Consider societal needs, problems that need solving, or ways to make a positive impact.</p>
        <p>What issues do you care about? Where do you see opportunities to contribute?</p>
    </div>
    """, 
...
# Input fields
st.markdown("### Share 3-5 needs or problems in the world you care about:")
...
for i in range(5):
    ...
    with col1:
        ...
        input_key = f"world_needs_input_{i}"
        input_value = st.text_input(f"Item {i+1}", key=input_key, value=st.session_state.world_needs_inputs[i])
        
        # Update session state when input changes
        if input_value != st.session_state.world_needs_inputs[i]:
            st.session_state.world_needs_inputs[i] = input_value
    ...
    with col2:
        ...
        if st.session_state.world_needs_suggestions[i]:
            for suggestion in st.session_state.world_needs_suggestions[i]:
...
with col2:
    ...
    with col_next:
        if st.button("Continue to What You Can Be Paid For →", key="continue_to_paid_for"):
            ...
            valid_inputs = [inp for inp in st.session_state.world_needs_inputs if inp.strip()]
            if valid_inputs:
